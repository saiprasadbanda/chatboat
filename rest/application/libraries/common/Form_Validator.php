...
class Form_Validator
{
    ...
    {
        $this->errors = array();
        $ruleset = $this->field_rules;

        foreach($ruleset as $field => $rules1)
        {
        ...
        return (count($this->errors) > 0)? $this->get_errors_array() : TRUE;
    }
        ...
        foreach($this->errors as $e) {
            ...
            switch($e['rule']) {
                ...
                case 'validate_max_numeric':
                    ...
                    break;
                case 'validate_match_field':
                    $resp[] = "The <span class=\"$field_class\">$field</span> field needs to be same as  $param";
                    break;
    ...
    public function get_errors_array($convert_to_string = null)
    {
        ...
        foreach($this->errors as $e) {
            ...
            $field = $e['field'];//ucwords(str_replace(array('_','-'), chr(32), $e['field']));
            $param = $e['param'];
            ...
            $rules = explode('validate_',$e['rule']);
            if(count($rules) > 1){
                $rule = $rules[1];
            }else{
                $rule = $rules[0];
            ...
            //echo "<br>".$param."<br>";
            $resp[$field] = $this->error_messages[$field][$rule];
            /*switch($e['rule']) {
                case 'mismatch' :
    ...
    */
    ...
    protected function validate_match_field($field, $input, $param = NULL)
    {
        if(isset($input[$field]) && isset($input[$param]) && !empty($input[$field]) && !empty($input[$param]))
        {
    ...
    public function add_rules($fieldName = '', $rules = '')
    {
        $validationData = array();
        $validationFields = array();
        $validationRules = array();
        $validationMessages = array();
        if(!empty($fieldName) && !empty($rules)){
            array_push($validationFields, $fieldName);
            $fieldRule = $rules;
            if(is_array($rules)){
                $fieldRule = '';
                $i = 0;
                ...
                foreach($rules as $key => $value){
                    ...
                    $rule = $key;
                    if($i+1 == $lnt){
                        $fieldRule = $fieldRule.$rule;
                    }else{
                        $fieldRule = $fieldRule.$rule.'|';
                    }
                    $i = $i+1;
                    $ar1[$rule] = $value;
                    $validationMessages[$fieldName][$rule] = $value;
                    $this->error_messages[$fieldName][$rule] = $value;
                }

            }

            $validationData[$fieldName] = $fieldRule;
            $validationRules[$fieldName] = $fieldRule;
            $this->field_rules[$fieldName] = $fieldRule;
        }

        //$validationData = array();
        //$validationFields = array();
        //$this->field_rules = $validationRules;
        //print_r($validationRules);
        //$this->error_messages = $validationMessages;
