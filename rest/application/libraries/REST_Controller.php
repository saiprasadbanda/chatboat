...
require APPPATH . 'third_party/excel.php';
...
abstract class REST_Controller extends CI_Controller {
    ...
    public function __construct($config = 'rest')
    {
        ...
        //getting language from client side
     ...
     * @param bool $continue TRUE to flush the`1     response to the client and continue
        ...
        elseif ($data !== NULL)
        {
           /* echo '<pre>';
            ...
            if (method_exists($this->format, 'to_' . $this->response->format))
            {
                ...
                if(DATA_ENCRYPT){
                    $result = [];
                    $result['responseData'] = $this->aesEncryption($output);
            ...
            //$output = $this->aesEncryption($output);
    ...
    protected function aesEncryption($requestData)
    {
        $aesObj = new AES();
        $data = $aesObj->encrypt($requestData,AES_KEY);
        //$data = (array) json_decode($data,true);
        return $data;
    }

    protected function aesDecryption($requestData)
    {
        $aesObj = new AES();
        $data = $aesObj->decrypt($requestData,AES_KEY);
        $data = (array) json_decode($data,true);
        return $data;
    }
    ...
    {
        if(isset($_GET['requestData']) && DATA_ENCRYPT)
           $_GET = $this->aesDecryption($_GET['requestData']);
    ...
    protected function _parse_post()
    {
       ...
       {
         $_POST = $this->aesDecryption($_POST['requestData']);
       }
    ...
    //custom functions
    private function oauth_verifier()
    {
        $apache_headers=apache_request_headers();
        $this->load->library('oauth/oauth');
        $_SERVER['HTTP_AUTHORIZATION']=isset($_SERVER['HTTP_AUTHORIZATION'])?$_SERVER['HTTP_AUTHORIZATION']:$apache_headers['Authorization'];
        $oauth = $this->oauth;
        $isRequestValid  = $oauth->verifyAccessToken();
        if(!$isRequestValid)
        {
            $this->response([
                $this->config->item('rest_status_field_name') => FALSE,
    ...
    public function checkUploadFileType1()
    {
        $flag = 0;
        ...
        $image_formats = array(
            ...
            'image/png',
            'image/gif',
            'image/bmp', 'image/x-windows-bmp','application/octet-stream'
        );
        ...
        if(isset($_FILES['file']['name']['profile_image']) || isset($_FILES['file']['name']['attachment']) || isset($_FILES['file']['name']['company_logo']) || isset($_FILES['file']['name']['branch_logo']) || isset($_FILES['file']['name']['country_flag']) || isset($_FILES['file']['name']['flag'])){
        ...
        {
            $file_types = array_values($_FILES['file']['type']);
            $file_sizes = array_values($_FILES['file']['size']);
            if(isset($file_types[0]) && is_array($file_types[0])){
                $file_types = $file_types[0];
            }
            if(isset($file_sizes[0]) && is_array($file_sizes[0])){
        ...
        if($flag==1)
        {
            $result = array('status'=>FALSE, 'error' => array('files' => $this->lang->line('invalid_format').' , Allowed Formats are '.join(', ',$allowed_types)), 'data'=>'');
            $this->response($result, REST_Controller::HTTP_OK);
        }
        else if($flag==2)
        {
            $result = array('status'=>FALSE, 'error' => array('files' => str_replace('%s',IMAGE_UPLOAD_SIZE,$this->lang->line('max_upload_size'))), 'data'=>'');
            $this->response($result, REST_Controller::HTTP_OK);
        }
    }
