...
class AuthCodeStorage extends AbstractStorage implements AuthCodeInterface
{
    ...
    {
        $this->db->get_where('oauth_auth_codes',array('auth_code' => $code, 'expire_time>=' => time()));
        $query = $this->db->get();
    ...
    public function create($token, $expireTime, $sessionId, $redirectUri)
    {
        ...
        $this->db->insert('oauth_auth_codes',$data);
    ...
    public function getScopes(AuthCodeEntity $token)
    {
        ...
        $this->db->from('oauth_auth_code_scopes');
        $this->db->join('oauth_scopes', 'oauth_auth_code_scopes.scope=oauth_scopes.id', 'left');
        $this->db->where('auth_code', $token->getId());
        $query = $this->db->get();
        $result = $query->result_array();
    ...
    {
        $this->db->insert('oauth_auth_code_scopes',array('auth_code' => $token->getId(), 'scope' => $scope->getId()));
    ...
    {
        $this->db->where('auth_code', $token->getId());
        $this->db->delete('oauth_auth_codes');
        /*Capsule::table('oauth_auth_codes')
                    ->where('auth_code', $token->getId())
                    ->delete();*/
    }
